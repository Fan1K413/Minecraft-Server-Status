...
export function TrendChart({ points, label = "24小时" }: { points: TrendPoint[]; label?: string }) {
  ...
  function nearest(event: React.PointerEvent<SVGSVGElement>): void { const rect = event.currentTarget.getBoundingClientRect(); const ratio = Math.max(0, Math.min(1, (event.clientX - rect.left) / rect.width)); const x = ratio * 100; const index = valid.reduce((best, current) => Math.abs(coordinate(current).x - x) < Math.abs(coordinate(best).x - x) ? current : best, valid[0]); setActiveIndex(index); }
  ...
  return <div className="chart-wrap"><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-label={`过去 ${label} 在线人数趋势，最高 ${maximum} 人`} tabIndex={0} onPointerMove={nearest} onPointerLeave={() => setActiveIndex(null)} onFocus={() => setActiveIndex(valid.at(-1) ?? null)} onKeyDown={keyDown}><path d={path} />{activePoint && <><line className="chart-guide" x1={activePoint.x} x2={activePoint.x} y1="0" y2="100" /><circle className="chart-dot" cx={activePoint.x} cy={activePoint.y} r="1.5" /></>}</svg>{active && active.playersOnline !== null && activePoint && <div className="chart-tooltip" style={{ left: `${Math.max(8, Math.min(92, activePoint.x))}%` }}><strong>{formattedTime(activeIndex!)}</strong><span>在线人数 {active.playersOnline}</span></div>}<span className="sr-only" aria-live="polite">{active && active.playersOnline !== null ? `${formattedTime(activeIndex!)}，在线人数 ${active.playersOnline}` : ""}</span><div className="chart-caption"><span>{label}前</span><strong>最高 {maximum} 人</strong><span>现在</span></div></div>;
}
