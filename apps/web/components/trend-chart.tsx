...
export function TrendChart({ points, label = "24小时" }: { points: TrendPoint[]; label?: string }) {
  ...
  if (!values.length) return <p className="empty-chart">尚未采集到在线人数数据。</p>;
  ...
  const activePoint = active && active.playersOnline !== null ? coordinate(activeIndex!) : null;
  const describe = (index: number) => new Intl.DateTimeFormat("zh-CN", { dateStyle: "medium", timeStyle: "short" }).format(new Date(points[index].at));
  function nearest(event: React.PointerEvent<SVGSVGElement>): void { const rect = event.currentTarget.getBoundingClientRect(); const ratio = Math.max(0, Math.min(1, (event.clientX - rect.left) / rect.width)); const raw = Math.round(ratio * (points.length - 1)); const index = valid.reduce((best, current) => Math.abs(current - raw) < Math.abs(best - raw) ? current : best, valid[0]); setActiveIndex(index); }
  ...
  return <div className="chart-wrap"><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-label={`过去 ${label} 在线人数趋势，最高 ${maximum} 人`} tabIndex={0} onPointerMove={nearest} onPointerLeave={() => setActiveIndex(null)} onFocus={() => setActiveIndex(valid.at(-1) ?? null)} onKeyDown={keyDown}><path d={path} />{activePoint && <><line className="chart-guide" x1={activePoint.x} x2={activePoint.x} y1="0" y2="100" /><circle className="chart-dot" cx={activePoint.x} cy={activePoint.y} r="1.5" /></>}</svg>{active && active.playersOnline !== null && activePoint && <div className="chart-tooltip" style={{ left: `${Math.max(8, Math.min(92, activePoint.x))}%` }}><strong>{describe(activeIndex!)}</strong><span>在线人数 {active.playersOnline}</span></div>}<span className="sr-only" aria-live="polite">{active && active.playersOnline !== null ? `${describe(activeIndex!)}，在线人数 ${active.playersOnline}` : ""}</span><div className="chart-caption"><span>{label}前</span><strong>最高 {maximum} 人</strong><span>现在</span></div></div>;
}
