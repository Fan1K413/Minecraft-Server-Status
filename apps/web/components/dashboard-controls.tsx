...
export function AvailabilityControl({ edition, initial }: { edition: "JAVA" | "BEDROCK"; initial: AvailabilityBucket[] }) {
  const [range, setRange] = useState<HistoryRange>("7d"); const [records, setRecords] = useState(initial); const [open, setOpen] = useState(false); const [loading, setLoading] = useState(false); const [error, setError] = useState(""); const menu = useRef<HTMLDivElement>(null);
...
export function PlayerTrendPanel({ initial }: { initial: TrendPoint[] }) {
  const [range, setRange] = useState<HistoryRange>("7d"); const [points, setPoints] = useState(initial); const [loading, setLoading] = useState(false);
  async function change(next: HistoryRange): Promise<void> { if (next === range) return; setLoading(true); try { const response = await fetch(`/api/v1/history?range=${next}`); if (!response.ok) throw new Error(); const data = await response.json() as { history: TrendPoint[] }; setPoints(data.history); setRange(next); } finally { setLoading(false); } }
  const label = ranges.find((item) => item.value === range)?.label ?? "7天";
  return <section className="section" aria-labelledby="trend-heading"><div className="section-heading"><div><p className="eyebrow">PLAYER TREND</p><h2 id="trend-heading">在线人数趋势</h2></div><div className="range-selector">{ranges.map((item) => <button key={item.value} type="button" className={item.value === range ? "range-active" : ""} onClick={() => void change(item.value)} disabled={loading}>{item.label}</button>)}</div></div><article className="chart-card"><TrendChart points={points} label={label} /></article></section>;
}
...
export function ProbeButton({ scope, label, className }: { scope: "JAVA" | "BEDROCK" | "all"; label: string; className: string }) {
  ...
  async function probe(): Promise<void> { setChecking(true); try { const response = await fetch("/api/v1/probe", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ scope }) }); const data = await response.json() as { results?: Record<string, { success: boolean; latencyMs?: number }>; success?: boolean; error?: string }; if (!response.ok) setResult(data.error === "too many checks" ? "检查过于频繁" : "暂时无法检查"); else { const results = data.results ?? (scope === "all" ? {} : { [scope]: data }); const passed = Object.values(results).filter((item) => item.success).length; setResult(scope === "all" ? `刚刚检查：${passed}/${Object.keys(results).length} 个入口可连接` : (data.success ? "刚刚检查：可连接" : "刚刚检查：无法连接")); } } catch { setResult("暂时无法检查"); } finally { setChecking(false); } }
