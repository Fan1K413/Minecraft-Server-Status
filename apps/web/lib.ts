...
import { overallStatus, publicEndpointStatus, type DailyAvailability, type OverallStatus } from "@minecraft-status/core";
...
export interface DashboardData {
  ...
  availability: { java: DailyAvailability[]; bedrock: DailyAvailability[] };
...
export async function getDashboardData(): Promise<DashboardData> {
  try {
    ...
    const availability = {
      java: config.java?.enabled ? database.getDailyAvailability("JAVA") : [],
      bedrock: config.bedrock?.enabled ? database.getDailyAvailability("BEDROCK") : [],
    };
    const now = new Date();
    ...
    database.close();
    return { config, java, bedrock, availability, overall: overallStatus(javaStatus, bedrockStatus, maintenance), checkedAt, configurationError: null };
  } catch (error) {
    return { config: null, java: null, bedrock: null, availability: { java: [], bedrock: [] }, overall: "UNKNOWN", checkedAt: null, configurationError: error instanceof Error ? error.message : "无法加载服务器配置" };
  }
}
