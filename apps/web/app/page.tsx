...
import { buildTrendWindow } from "@minecraft-status/core";
...
export default async function Page() {
  const data = await getDashboardData(); const database = new StatusDatabase(); const trendTo = new Date(); const trendFrom = new Date(trendTo.getTime() - 72 * 3_600_000); const trend = data.config ? buildTrendWindow(database.getHistory(72, trendTo), trendFrom, trendTo, data.config.monitor.intervalSeconds, 360) : []; const availability = { java: database.getAvailabilityBuckets("JAVA", 72), bedrock: database.getAvailabilityBuckets("BEDROCK", 72) }; database.close();
  ...
  return <main className="shell"><AutoRefresh /><header className="site-header"><div><p className="eyebrow">MINECRAFT SERVER</p><h1>{data.config.server.name}</h1></div><div className="overall-status"><ProbeButton scope="all" label={labels[data.overall]} className={data.overall.toLowerCase()} /><time className="checked-time" dateTime={data.checkedAt ?? undefined}>{checkedAt}</time></div></header>
    ...
    <PlayerTrendPanel initial={trend} initialFrom={trendFrom.toISOString()} initialTo={trendTo.toISOString()} />
