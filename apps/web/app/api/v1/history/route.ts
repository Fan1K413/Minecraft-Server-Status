...
import { downsampleTrend, historyRangeHours, markTrendGaps, parseHistoryRange } from "@minecraft-status/core";
...
export async function GET(request: NextRequest) {
  ...
  const history = downsampleTrend(markTrendGaps(database.getHistory(rangeHours), config.monitor.intervalSeconds), maxPoints);
