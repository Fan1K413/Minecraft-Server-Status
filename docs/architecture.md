...
SQLite 启用 WAL 与 `busy_timeout=5000`，启动时会自动执行带 `PRAGMA user_version` 的事务迁移。当前有四张表：

| 表 | 用途 |
| --- | --- |
| `endpoint_snapshots` | Java / Bedrock 各一行的当前状态、计数和最新展示数据。 |
| `check_results` | 每次检查的原始成功/失败、延迟、玩家数和错误分类；不聚合。 |
| `endpoint_metadata_events` | 仅在 Java 的版本、规范化 MOTD 或 favicon 发生变化时追加的永久元数据历史。 |
| `favicon_blobs` | 以完整 favicon data URI 的 SHA-256 哈希去重保存的图标内容。 |

`check_results` 仍按 `retentionDays` 清理旧的原始检查记录。版本、MOTD 和图标变更历史永久保留；图标仅在没有被快照或任何变更事件引用时才会清理。当前 schema 保留旧历史列以便无损升级，但新检查不再把版本、MOTD 或 Base64 favicon 重复写入每一行。没有 `servers`、`daily_stats` 或 `incidents` 表。
...
- Java 使用与 Monitor 相同的 Java Server List Ping；基岩执行 UDP ping。
- 基岩执行 UDP ping。
